    ...
    def test_tokens(self):
        ...
        for index, site in enumerate(fake_site_names):
            ...
            for clazz in fake_using_agent_classes:
                choices = [None, clazz.generate_fake_tokens(fake)]
                if index > 0:
            ...
            test_token_table[site] = fake_token_set

        for clazz in get_all_api_classes():
            for site_name, tokens_set in test_token_table.items():
                clazz().get_agent_tokens(site_name, test_token_table)
